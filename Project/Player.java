...
public class Player{
	...
	public Card[] discard(){
		...
		for(int i = 0; i<5; i++){
			String s = ""+ i +": " + this.playerHand.getCard(i);
			System.out.println(s);
		}
		
		System.out.println("How many cards would you like to discard?");
		int numOfDiscardedCards = 1;
		
		Card[] discardedCards = new Card[numOfDiscardedCards];
		
		int indexOfDiscardedCard = 0; 
		
		for(int i = 0; i< numOfDiscardedCards; i++){
			discardedCards[i] = this.playerHand.getCard(indexOfDiscardedCard);
			
			this.playerHand.removeCard(indexOfDiscardedCard);
	...
	public double wager(double min){
		...
		double wager=0.0;
		if(this.balance < min || wager > this.balance){
			return -1.0;
		}
